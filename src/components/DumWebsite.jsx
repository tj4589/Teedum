...
export default function DumWebsite() {
  ...
  const spin = () => {
    ...
    const targetRotation = spinDeg + extraSpins;
  ...
  const reset = () => { setStage("select-day"); setChecked([]); setSelectedDay(""); };

  return (
    <div className="dum-website-wrapper">
      <style>{`
        ...
        .dum-website-wrapper * { box-sizing: border-box; }
        ...
        .btn-primary-dum:hover { transform: translateY(-2px); box-shadow: 0 15px 30px rgba(139, 92, 246, 0.3); }
        .btn-primary-dum:active { transform: scale(0.98); }
        ...
        .dum-header { text-align: center; margin-bottom: 3rem; }
        .dum-header h1 {
          font-family: 'Playfair Display', serif;
        ...
        .dum-header p { color: var(--purple-main); font-weight: 600; letter-spacing: 1px; font-size: 0.9rem; }
        ...
        <div className={`lock-container ${success ? "fading" : ""}`}>
          <div className={`lock-card ${shake ? "shaking" : ""}`}>
            ...
            <input
              ...
              disabled={success}
            />
            <button className="btn-primary-dum" onClick={handleUnlock} disabled={success}>
      ...
      {/* ── MAIN APP ── */}
      {unlocked && (
        <div className="dum-app-container">
          <header className="dum-header" style={{ animation: "popIn 0.8s ease" }}>
          ...
          {(stage === "wheel" || stage === "spinning") && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
              ...
              {!spinning && (
                <button className="btn-primary-dum" style={{ width: "auto", padding: "18px 48px" }} onClick={spin}>
          ...
          {stage === "result" && (
            <div className="result-card">
              <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                {activities.map(a => (
                  <span key={a.id} className="result-icon" style={{ marginBottom: 0 }}>{a.icon}</span>
                ))}
              </div>
              <h2 className="result-title">Sike! We are doing it all, get ready my baby</h2>
              <p className="result-subtitle">For {selectedDay} Night</p>
              <p className="result-desc">I can't wait, it's going to be a maza 🎉</p>
